        ...
        for point in self.points:
            for neighbor_y in range(-1, 2):
                for neighbor_x in range(-1, 2):
                    neighbor_point = board[point[0] + neighbor_y][point[1] + neighbor_x]
...
class Country:
    ...
    def set_near_countries(self, board):
        for y in range(-100, 100):
            for x in range(-100, 100):
                point = board[self.center[0] + y][self.center[1] + x]
                if point != 0 and countries[point[0]] != self and countries[point[0]] not in self.near_countries:
                    self.near_countries.append(countries[point[0]])
